...
func getCart(c *gin.Context) {
	...
	row := stmt.QueryRow(currentUser, constant.StatusActive)
...
func CreateCart(c *gin.Context, username string) (success bool) {
	...
	result, err := tx.Exec("INSERT INTO cart(create_time, status, username) VALUES ($1, $2, $3)", time.Now(), constant.StatusActive, username)
	if err != nil {
...
func GetCartProducts(c *gin.Context, cartId uint64) (cartProducts *map[string]*uint8) {
	...
	rows, err := stmt.Query(cartId, constant.StatusActive)
	if err != nil {
		middleware.InternalServer(c, err)
		return
	}
...
func addCartProduct(c *gin.Context) {
	...
	row := stmt.QueryRow(currentUser, constant.StatusActive)
	err = row.Scan(&id)
	var newCartProducts []*model2.CartProduct
	...
	for _, product := range newCartProducts {
		result, err = stmt.Exec(id, *product.ProductCode, *product.Quantity, constant.StatusActive)
...
func deleteCartProduct(c *gin.Context) {
	...
	var result sql.Result
	var count int64
	result, err = stmt.Exec(id, pq.Array(productCodes), constant.StatusRemoved)
	if count, err = result.RowsAffected(); err != nil {
		middleware.InternalServer(c, err)
		return
	}
